...
async def get_address_coords(address: str) -> dict:
    async with Nominatim(
        ...
        location = await geolocator.geocode("747 Howard St, San Francisco, CA 94103")
        return {
...
def filter_by_proximity(center: dict, radius: float, data: dict):
    ...
    p2 = gpd.GeoSeries([point], crs="EPSG:4326")
    p2_cartesian = p2.to_crs(crs="EPSG:3857").iloc[0]
    gdf_cartesian = gdf.to_crs(crs="EPSG:3857")
    gdf["proximity"] = gdf_cartesian.distance(p2_cartesian)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--block-length", required=False, default=182.88, type=float, help="Length in meters to measure a block. Defaults to US average of 182.88m.")
